...
class BraiinsPoolAPIConfig(PoolAPIConfig):
    ...
    def from_args(cls, args) -> "BraiinsPoolAPIConfig":
        ...
        if not api_key:
            raise ValueError(
                "Braiins API key must be provided via --pool.api_key or "
                "BRAIINS_API_KEY env var."
            )
...
class BraiinsPoolConfig:
    ...
    def add_args(cls, parser: "argparse.ArgumentParser") -> None:
        ...
        parser.add_argument(
            ...
            help=f"Pool domain (env: BRAIINS_POOL_DOMAIN, default: {
                cls.DEFAULT_DOMAIN
            })",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            help=f"Pool worker password (env: BRAIINS_WORKER_PASSWORD, default: '{
        ...
        parser.add_argument(
            ...
            required=False,
            default=(
                int(os.getenv("BRAIINS_HIGH_DIFF_PORT"))
                if os.getenv("BRAIINS_HIGH_DIFF_PORT")
                else None
            ),
            help=(
                "Optional proxy port for"
                "high difficulty miners (env: BRAIINS_HIGH_DIFF_PORT)"
            ),
        )
    ...
    def from_args(cls, args) -> "BraiinsPoolConfig":
        ...
        if not username:
            raise ValueError(
                "Braiins pool username must be provided via --pool.username or "
                "BRAIINS_POOL_USERNAME env var."
            )
