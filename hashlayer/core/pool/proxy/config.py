...
from hashlayer.core.chain_data.pool_info import PoolInfo
from hashlayer.core.pool.config import PoolAPIConfig
from hashlayer.core.pool.pool import PoolIndex
...
class ProxyPoolAPIConfig(PoolAPIConfig):
    ...
    def add_args(cls, parser: "argparse.ArgumentParser") -> None:
        ...
        parser.add_argument(
            ...
            help=(
                f"Proxy API port (env: PROXY_API_PORT, default: {cls.DEFAULT_API_PORT})"
            ),
        )
        parser.add_argument(
    ...
    def from_args(cls, args) -> "ProxyPoolAPIConfig":
        ...
        if not api_token:
            raise ValueError(
                "Proxy API token must be provided via --pool.proxy_api_token or "
...
class ProxyPoolConfig:
    ...
    def add_args(cls, parser: "argparse.ArgumentParser") -> None:
        ...
        parser.add_argument(
            ...
            help=(
                "[DEPRECATED] Use --pool.domain instead."
                "Proxy IP address (env: PROXY_IP)"
            ),
        )
        parser.add_argument(
            "--pool.port",
            ...
            help=(
                f"Proxy port for normal miners (env: PROXY_PORT, "
                f"default: {cls.DEFAULT_PORT})"
            ),
        )
        parser.add_argument(
            "--pool.username",
        ...
        parser.add_argument(
            ...
            help=f"Pool worker password (env: PROXY_PASSWORD, default: '{
        ...
        parser.add_argument(
            ...
            help=(
                f"Proxy port for high difficulty miners "
                f"(env: PROXY_HIGH_DIFF_PORT, default: {cls.DEFAULT_HIGH_DIFF_PORT})"
            ),
        )
    ...
    def from_args(cls, args) -> "ProxyPoolConfig":
        ...
        if not domain:
            raise ValueError(
                "Proxy domain/IP must be provided via --pool.domain "
                "(or PROXY_DOMAIN/PROXY_IP env var)"
            )

        username = args.pool.username
        if not username:
            raise ValueError(
                "Pool username must be provided via --pool.username or "
                "PROXY_USERNAME env var."
            )
