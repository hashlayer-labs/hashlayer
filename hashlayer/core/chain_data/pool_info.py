...
from hashlayer.core.utils import ip_to_int
...
class PoolInfo:
    ...
    def high_diff_pool_url(self) -> str:
        ...
        If high_diff_port is set,
            uses that port. Otherwise falls back to regular pool_url.
...
def publish_pool_info(
    subtensor: Subtensor, netuid: int, wallet: "Wallet", pool_info_bytes: bytes
) -> bool:
    """
...
def get_all_pool_info(
    subtensor: Subtensor, netuid: int, valid_hotkeys: list[str]
...
def get_pool_info(subtensor: Subtensor, netuid: int, hotkey: str) -> Optional[PoolInfo]:
    """
    ...
    try:
        ...
        try:
            ...
            logging.debug(
                f"Failed to decode pool info (might be miner commitment): {e}"
            )
            return None
