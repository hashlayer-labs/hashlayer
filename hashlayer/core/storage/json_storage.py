...
from hashlayer.core.constants import MAIN_PATH
from hashlayer.core.storage.base_storage import BaseStorage
from hashlayer.core.storage.utils import check_key, extract_block_number
...
class BaseJsonStorage(BaseStorage):
    ...
    def get_latest(self, prefix: str = "pools") -> Optional[Any]:
        ...
        Returns:
            The deserialized JSON data from the latest matching file, or
        ...
        try:
            ...
            logging.error(
                (
                    f"No [red]{prefix}[/red] files found in"
                    f"[blue]{dynamic_files_path.as_posix()}[/blue]."
                )
            )
            return None
