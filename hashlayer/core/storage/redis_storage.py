...
from hashlayer.core.storage.base_storage import BaseStorage
from hashlayer.core.storage.utils import check_key, dumps, loads
...
class BaseRedisStorage(BaseStorage):
    ...
    def load_data(self, key: Optional[Any], prefix: str = "pools") -> Optional[dict]:
        """
        Loads and retrieves data from a client storage based on a key and
        optional prefix.

        Parameters:
            key (str): The key to locate the data in the client storage.
            prefix (Optional[str]): An optional prefix to prepend to the
                key for locating the data.

        Returns:
            Optional[dict]: The deserialized data fetched from the client storage, or
                None
    ...
    def get_latest(self, prefix: str = "pools") -> Optional[Any]:
        ...
        Returns:
            latest_block_data: related with key:value as
                {prefix:latest_block}:latest_block_data
