...
from hashlayer.core.pricing.coingecko import CoinGeckoAPI
from hashlayer.core.pricing.coinmarketcap import CoinMarketCapAPI
from hashlayer.core.pricing.hash_price import BraiinsHashPriceAPI
from hashlayer.core.pricing.price import (
    CoinPriceAPIBase,
    HashPriceAPIBase,
    UnitCoinPriceAPI,
)
...
class CoinPriceAPI:
    ...
    def __new__(cls, method: str, api_key: Optional[str]) -> "CoinPriceAPIBase":
        ...
        if method not in cls.__CLASS_MAP:
            raise ValueError(
                f"Unknown price method: {method}. Available methods: {
                    list(cls.__CLASS_MAP.keys())
                }"
            )
...
class HashPriceAPI:
    ...
    def __new__(cls, method: str = "braiins") -> "HashPriceAPIBase":
        ...
        if method not in cls.__CLASS_MAP:
            raise ValueError(
                f"Unknown hash price method: {method}. Available methods: {
                    list(cls.__CLASS_MAP.keys())
                }"
            )
